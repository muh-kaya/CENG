...
int main(int argc, char **argv)
{
	...
	while(1){
		...
		/*for(i=0; i<coming.object_count; i++){
			if(coming.adv_pos.x == coming.object_pos[i].x && coming.adv_pos.y == coming.object_pos[i].y){
				prey = 1;
				break;
			}
		}
		...
		if(prey == 1){
			going.move_request.x = coming.object_pos[i].x;
			going.move_request.y = coming.object_pos[i].y;
		}
